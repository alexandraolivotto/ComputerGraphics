...
void Laborator4::Init()
{
	...
	angularStepOX_jump = 180;
	angularStepOY_jump = 0;
	angularStepOZ_jump = 0;
}
...
void Laborator4::Update(float deltaTimeSeconds)
{
	...
	modelMatrix *= Transform3D::Translate(translateX_jump, translateY_jump, translateZ_jump);
	modelMatrix *= Transform3D::RotateOX(angularStepOX_jump);
	modelMatrix *= Transform3D::RotateOY(angularStepOY_jump);
	modelMatrix *= Transform3D::RotateOZ(angularStepOZ_jump);
	RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
	JumpingBox(deltaTimeSeconds);
}
...
void Laborator4::JumpingBox(float deltaTime) {
	translateX_jump += deltaTime;
	if (angularStepOX_jump == 180) {
		angularStepOX_jump -= deltaTime;
		angularStepOY_jump -= deltaTime;
		angularStepOZ_jump -= deltaTime;
	}
	else 
	if (angularStepOX_jump == 0) {
		angularStepOX_jump += deltaTime;
		angularStepOY_jump += deltaTime;
		angularStepOZ_jump += deltaTime;
	}
