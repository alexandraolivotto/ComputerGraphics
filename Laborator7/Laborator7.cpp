...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab7"], modelMatrix, glm::vec3(1, 0, 0.3));
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab7"], modelMatrix, glm::vec3(0.1, 1, 0.4));
	}
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab7"], modelMatrix, glm::vec3(0.1, 0.1, 0.2));
	}
...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	//shininess
	GLint material = glGetUniformLocation(shader->program, "material_shininess");
	...
	//color
	GLint matColor = glGetUniformLocation(shader->program, "object_color");
	glUniform3fv(matColor, 1, glm::value_ptr(color));
