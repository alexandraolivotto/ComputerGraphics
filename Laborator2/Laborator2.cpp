...
void Laborator2::Init()
{
	...
	// task4: create a mesh tetrahedron using custom data
	{
		//4 triangles and 4 vertices
		vector<VertexFormat> vertices
		{
			VertexFormat(glm::vec3(-0.5, 0,  0), glm::vec3(0, 0, 1)), //B
			VertexFormat(glm::vec3(0.5f, 1,  0), glm::vec3(1, 0, 1)), //A
			VertexFormat(glm::vec3(0, 0,  1), glm::vec3(0, 1, 0)), //C
			VertexFormat(glm::vec3(1, 0,  0), glm::vec3(1, 0, 0)) //D
		};
		...
		Mesh* th = CreateMesh("th1", vertices, indices);
	}

	//task6 - create a square
	{
		//2 triangles and 4 vertices
		vector<VertexFormat> vertices
		{
			VertexFormat(glm::vec3(1, 2,  0), glm::vec3(0, 0, 1)), //A
			VertexFormat(glm::vec3(2, 2,  0), glm::vec3(1, 1, 0)), //B
			VertexFormat(glm::vec3(2, 1,  0), glm::vec3(0, 1, 0)), //C
			VertexFormat(glm::vec3(1, 1,  0), glm::vec3(1, 0, 0)) //D
		};
		...
		//task4 - create another mesh
		Mesh* sq = CreateMesh("square", vertices, indices);
	}

	// bonus - creat a circle 
	{
		vector<VertexFormat> vertices;
		vector<unsigned short> indices;
		float arg;
		//point on the circle <==> x^2 + y^2 = r^2; let r = 1 => x^2 + y^2 = 1 => x = sin arg, y = cos arg.
		//arg = 2pi r/ circle_lines 

		vertices.emplace_back(glm::vec3(0, 0, 0), glm::vec3(1.f, 1.f, 1.f));
		for (int i = 0; i <= circle_lines; i++)
		{
			//circle length / number of lines
			arg = 2*M_PI*i / circle_lines;

			vertices.emplace_back(glm::vec3(sin(arg), cos(arg), 0), glm::vec3(1.f, 1.f, 0.f));
			indices.push_back(i);
		}

		// Create a new mesh from buffer data
		Mesh* cr = CreateMesh("circle", vertices, indices);
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	//task2 - show cube3
	RenderMesh(meshes["cube3"], shaders["VertexColor"], glm::vec3(1.5f, 1, 0), glm::vec3(0.25f));
	//task4 - show the tetrahedron
	RenderMesh(meshes["th1"], shaders["VertexColor"], glm::vec3(0, 0, 0.5f), glm::vec3(0.25f));
	...
	//render a square
	RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(1, 1, 1), glm::vec3(0.25f));

	//render a circle
	RenderMesh(meshes["circle"], shaders["VertexColor"], glm::vec3(-0.5f, 2, 0), glm::vec3(0.15f));
